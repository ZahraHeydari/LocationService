...
private const val QUOTA_EXCEEDED_ERROR_MESSAGE = "Quota Exceeded!"
private const val NO_CONNECTION_ERROR_MESSAGE = "No Connection!"
private const val SQLITE_ERROR_MESSAGE = "Sqlite error!"
...
data class ApiError(val message: String?, val code: Int?, var errorStatus: ErrorStatus) {
    ...
    fun getErrorMessage(): String {
        return when (errorStatus) {
            ...
            ErrorStatus.QUOTA_EXCEEDED -> QUOTA_EXCEEDED_ERROR_MESSAGE
            ErrorStatus.NO_CONNECTION -> NO_CONNECTION_ERROR_MESSAGE
            ErrorStatus.SQLITE_EXCEPTION -> SQLITE_ERROR_MESSAGE
    ...
    /**
     * Various error status based on FourSquare document to know what happened if something goes wrong with a repository call
     */
